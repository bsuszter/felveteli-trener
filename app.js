...
function score() {
  return state.answers.reduce((sum, answer, index) => sum + (answer && isCorrect(TASKS[index], answer) ? 1 : 0), 0);
}
...
function renderQuestion() {
  ...
  app.innerHTML = `
    <section class="question">
      <div class="question-head"><span>${state.index + 1}. feladat</span><span>1 pont</span></div>
      <p class="task-source">${task.source}</p>
      <h2>Melyik alak helyes?</h2>
      <div class="word-pair"><div class="word"><span>${task.first}</span></div><div class="word"><span>${task.second}</span></div></div>
...
function practiceTopicForTask(task) {
  if (task.tags.includes("felszólító mód")) return "felszólító mód";
  if (task.tags.includes("igekötő")) return "igekötők";
  if (task.tags.includes("kis- és nagybetű") || task.tags.includes("történelmi esemény") || task.tags.includes("földrajzi név")) return "kis- és nagybetű";
  return null;
}
...
function buildPracticeSet() {
  let topics = getRecommendedTopics();
  if (!topics.length) topics = ["felszólító mód", "igekötők", "kis- és nagybetű"];
  const selected = [];
  topics.slice(0, 2).forEach(topic => selected.push(...PRACTICE_TASKS.filter(item => item.topic === topic).slice(0, 2)));
  PRACTICE_TASKS.forEach(item => { if (selected.length < 4 && !selected.includes(item)) selected.push(item); });
...
function renderResult() {
  ...
  app.innerHTML = `
    <section class="result">
      ...
      ${weakTags.length ? `<div class="focus-block"><p>Érdemes még gyakorolnod:</p><div class="tag-list">${weakTags.map(tag => `<span class="topic-tag">${tag}</span>`).join("")}</div></div>` : `<p class="lead">Minden területen biztosan teljesítettél.</p>`}
      <div class="practice-offer"><span class="practice-icon">✦</span><div><h3>${recommended.length ? "Személyre szabott gyakorlás" : "Bónusz gyakorlás"}</h3><p>${recommended.length ? `A hibáid alapján főként ezt gyakorolnád: <strong>${recommended.slice(0, 2).join(" és ")}</strong>.` : "Hibátlan lett a feladatsor, de négy bónuszpéldával még biztosabbá teheted a tudásodat."}</p></div></div>
      <div class="result-actions"><button class="primary" id="practiceBtn">4 feladatot gyakorlok</button><button class="secondary" id="reviewBtn">Válaszaim áttekintése</button><button class="secondary" id="restartBtn">Újrakezdem</button></div>
    </section>`;
...
function startPractice() {
  state.practiceTasks = buildPracticeSet();
...
function renderPractice() {
  ...
  app.innerHTML = `
    <section class="practice-screen">
      <div class="question-head"><span>Személyre szabott gyakorlás</span><span>${state.practiceIndex + 1} / ${state.practiceTasks.length}</span></div>
      <span class="topic-tag practice-topic">${task.topic}</span>
      <h2>${task.prompt}</h2>
      ...
      ${answered ? `<div class="practice-feedback ${correct ? "correct" : "wrong"}"><strong>${correct ? "Helyes!" : "Ezt most nézzük meg."}</strong><p>${task.explanation}</p></div>` : ""}
      <div class="navigation"><button class="nav-btn" id="quitPractice">← Eredmény</button><button class="nav-btn next" id="nextPractice" ${answered ? "" : "disabled"}>${state.practiceIndex === state.practiceTasks.length - 1 ? "Gyakorlás befejezése" : "Következő →"}</button></div>
    </section>`;
  ...
  document.getElementById("quitPractice").addEventListener("click", renderResult);
...
function getWeakTags() {
  const counts = {};
  TASKS.forEach((task, index) => { if (!isCorrect(task, state.answers[index])) task.tags.forEach(tag => counts[tag] = (counts[tag] || 0) + 1); });
